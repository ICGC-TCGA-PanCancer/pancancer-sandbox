...
if ($use_nova) {
  ...
  my $nova_str = `nova list | grep fleet_master | grep ACTIVE | awk '{print \$2" "\$4" "\$8}'`;
  if ($glob_target ne "") {
    $nova_str = `nova list | grep $glob_target | grep ACTIVE | awk '{print \$2" "\$4" "\$8}'`;
  }

  print "nova list | grep $glob_target | grep ACTIVE | awk '{print \$2\" \"\$4\" \"\$8}'\n";
